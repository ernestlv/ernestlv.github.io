   ...
   var Controller = BaseController.extend("Controller",
        {
            ...
            clientInit: function (sessionExpiration) {
                ...
                if (top.document.location.search.match(/test=(.*)/))
                    this.load(RegExp.$1).then(function () {
                        this.play()
                    }.bind(this), function (m) {alert(m)});
            },
            play: function () {
                if (this.isPlayable()) {
                    this.clear();
                    ...
                    for (var ix = this.currentIx; ix < this.maxIx;++ix) {
                        ...
                        // Try and process directive
                         ...
                         if( obj.type === 'wait' ){
                             ...
                             // Wait time can not be smaller than time interval, the real delay time should be larger one of this.time_interval and obj.value
                             var delay = 0;
                             if ( obj.value > time_interval ){
                                 delay = obj.value - time_interval;
                             }
                             setTimeout(function(){
                                 this.interval = setInterval(function () {
                                     this.tick();
            ...
            load: {on: "server", body: function (test) {
                test = test.replace(/[^A-Za-z0-9_]/g, '');
                var file = __dirname + '/../../../../testscripts/' + test + '.js';
                try {
                    this.uploadedFile = fs.readFileSync(file).toString();
                    this.status = 'ready';
                } catch (e) {
                    throw e.message + " loading " + file;
                }
